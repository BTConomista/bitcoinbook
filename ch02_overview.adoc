...
// [REVISIONE]: l'originale dice "the difference represents an implied _transaction fee_" (commissione di transazione _implicita_). La traduzione omette sia "implicita" (implied) sia "di transazione", rendendo solo "_commissione_". Il termine "implied" è tecnicamente rilevante: la commissione non è dichiarata esplicitamente nella transazione ma si ricava come differenza tra input e output. Preferire "commissione di transazione _implicita_".
Le transazioni si possono immaginare come righe di un registro a partita doppia. Ogni transazione include uno o più input, che spendono fondi, e uno o più output, che li ricevono. Di norma, la somma degli output è leggermente inferiore a quella degli input: la differenza è rappresentata dalla _commissione_ incassata dal miner che include la transazione nella blockchain. La figura <<transaction-double-entry>> mostra una transazione come registrazione in un libro mastro basato sul metodo della partita doppia.
...
Un wallet che gira su un full node mantiene l'intero set degli output non spesi (UTXO) di ogni transazione confermata. Poiché però i full node richiedono molte risorse, molti wallet utente girano come client leggeri che registrano solo gli UTXO dell'utente.
...
// [REVISIONE — DA RIVEDERE]: l'originale tecnico ("Alice will therefore _encumber_ the output value with a demand for a signature from Bob") perde il concetto chiave di _encumber_ (vincolare l'output a una condizione di spesa); la traduzione "Alice quindi lega il valore dell'output alla firma di Bob" attenua il concetto e il paragrafo successivo ("In altre parole Bob dovrà firmare...") inverte il punto di vista (Bob deve firmare invece di Alice che vincola). Valutare una resa più aderente al concetto di "encumbrance" (es. "vincola l'output", "lega l'output a una condizione di spesa") e mantenere il soggetto del periodo originale.
...
// [REVISIONE]: nel primo punto della lista che segue, l'originale "Miners can only receive *honest income* from creating blocks that follow all of Bitcoin's _consensus rules_" perde la qualificazione "honest" nella resa "I miner possono ricevere la loro ricompensa solo se...". L'aggettivo "honest" è concettualmente importante perché distingue il guadagno da blocchi validi (onesto) dal possibile profitto da comportamenti disonesti (riferimento esplicito poco oltre, "miners who behave honestly" / "questo comportamento disonesto"). Suggerito: "I miner possono ricevere un guadagno onesto solo se..." o equivalente.
...
// [REVISIONE]: errore numerico — la formula 1,68 × 10^23 è corretta (= 168 × 10^21), ma il parentetico "168 miliardi di miliardi di miliardi" rende (10^9)^3 = 10^27, cioè 1,68 × 10^29 (sei ordini di grandezza in più). L'inglese "168 billion trillion" significa 168 × 10^9 × 10^12 = 168 × 10^21 (scala corta US). Suggerito: "168 mila miliardi di miliardi" oppure "168 triliardi" (long scale italiana, 10^21), oppure rimuovere il parentetico e lasciare solo la notazione scientifica.
// [REVISIONE — alt. proposta da sessione 84pWk]: stesso errore numerico, ma con correzione in scala corta: "168 miliardi di trilioni" (miliardi = 10^9, trilioni = 10^12). NB: in italiano standard "trilione" è 10^18 (scala lunga), non 10^12 (scala corta US): la proposta in scala lunga sopra è linguisticamente più corretta. Decisione finale al revisore umano.
Se l'output della funzione di hash corrisponde a un modello stabilito dal protocollo, il miner vince la lotteria e il blocco viene accettato come valido. In caso contrario modifica leggermente il blocco candidato e riprova. Nel periodo di questa stesura, in media un miner deve provare circa 1,68 × 10^23^ (168 miliardi di miliardi di miliardi di) blocchi candidati, e altrettante volte deve eseguire la funzione di hash, prima di trovare la combinazione vincente.
...
// [REVISIONE]: l'originale dice "prioritized by the highest fee rate transactions first" — non "highest fee". In Bitcoin il criterio di priorità è il *fee rate* (commissione divisa per peso/dimensione), non il valore assoluto della commissione. La traduzione "commissioni più alte" perde questa distinzione tecnica fondamentale (rilevante per RBF/CPFP discusso in ch09). Suggerito: "dando priorità a quelle con il tasso di commissione (fee rate) più alto".
Le transazioni vengono aggiunte al nuovo blocco dando priorità a quelle con commissioni più alte e secondo altri criteri. Ogni miner inizia a minare un nuovo blocco candidato appena riceve il blocco precedente dal network, consapevole che un altro miner ha vinto quella tornata della lotteria. I miner creano subito un nuovo blocco candidato collegandolo al blocco precedente, lo riempiono di transazioni e iniziano a calcolare la PoW per il blocco candidato.
...
// [REVISIONE]: l'originale dice "the pool of *unverified* transactions" (transazioni non ancora verificate/validate); la traduzione "transazioni non confermate" sposta il concetto a "unconfirmed" (non ancora incluse in un blocco), che è una proprietà diversa. Tecnicamente la mempool contiene transazioni "unverified" finché il nodo non le valida, e poi rimangono "unconfirmed" finché non finiscono in un blocco. Suggerito: "transazioni non verificate".
La transazione di Alice è stata propagata nella rete, finendo nel pool delle transazioni non confermate. Un full node l'ha poi validata e inserita in un blocco candidato.
