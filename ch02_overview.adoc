...
// [REVISIONE]: attenuazione di concetto chiave — l'originale dice "does not require trust in third parties" (non richiede fiducia in terze parti). La traduzione semplifica con "non dipende da intermediari", perdendo il riferimento al modello trustless centrale in Bitcoin. Suggerito: "non richiede di fidarsi di terze parti".
...
// [REVISIONE]: incoerenza terminologica all'interno dello stesso capitolo — qui e a riga 91 si usa "blockchain explorer" mentre a righe 26, 29 e 225 si usa "block explorer". L'originale usa coerentemente "block explorer". Uniformare la scelta in tutto il capitolo.
...
// [REVISIONE]: errori multipli — refusi "Detenere  tuoi primi" (doppio spazio, manca "i") e "per2 bitcoin" (manca lo spazio); inoltre "scambiato contanti per 2 bitcoin" introduce una quantità ("2 bitcoin") non presente nell'originale ("to exchange some cash for bitcoins"): correggere quantità e refusi.
Alice, introdotta nel capitolo precedente, ha appena acquistato i suoi primi bitcoin. Nel paragrafo "Detenere  tuoi primi bitcoin" <<getting_first_bitcoin>>, Alice ha incontrato il suo amico Joe e ha scambiato contanti per2 bitcoin. Da allora ne ha comprati altri e ora è pronta alla sua prima spesa: acquistare l'accesso a un episodio premium di un podcast dal negozio online di Bob.
...
// [REVISIONE]: virgola fuori posto: "(e l'indirizzo a cui viene inviato, è chiamato _change address_, cioè indirizzo di resto)" spezza il soggetto dal verbo. L'originale è una sola frase ("the address it pays, called a _change address_"). Riformulare ad es. "(e l'indirizzo a cui viene inviato è chiamato _change address_, cioè indirizzo di resto)".
...
// [REVISIONE]: errore tecnico — l'ID dell'ancora è stato tradotto da `transaction-common` a `transazione-comune`, ma la cross-reference subito sopra (`<<transaction-common>>`) usa ancora l'ID inglese: il riferimento risulta rotto. Ripristinare `[[transaction-common]]` (gli ID non vanno tradotti).
[[transazione-comune]]
...
// [REVISIONE]: errore tecnico — l'originale dice "many user wallets run lightweight clients that track only the user's own UTXOs", cioè molti wallet utente *sono* essi stessi client leggeri. La traduzione "molti wallet si appoggiano a client leggeri" suggerisce invece una dipendenza da un client leggero esterno. Suggerito: "molti wallet utente girano come client leggeri che registrano solo gli UTXO dell'utente".
Un wallet che gira su un full node mantiene l'intero set degli output non spesi (UTXO) di ogni transazione confermata. Poiché però i full node richiedono molte risorse, molti wallet si appoggiano a client leggeri che registrano solo gli UTXO dell'utente.
...
// [REVISIONE]: omissione esplicativa — l'originale dice "...visualized as edges (lines) in a graph, with the peers themselves being the nodes (dots). For that reason, Bitcoin peers are commonly called 'full verification nodes'". L'inciso "with the peers themselves being the nodes (dots)" è omesso: nell'italiano l'argomentazione passa direttamente da "archi in un grafo" a "per questo... vengono chiamati nodi", senza spiegare che, nella stessa metafora del grafo, i peer sono i puntini (= nodi). Inoltre il paragrafo apre con "I nodi della rete peer-to-peer" mentre l'originale dice "Peers": il termine "nodi" viene usato prima di essere giustificato. Suggerito: ripristinare l'inciso (es. "...rappresentate come archi (linee) in un grafo, mentre i peer stessi corrispondono ai nodi (puntini); per questo i peer di Bitcoin sono chiamati 'nodi di verifica completa'..." ) e considerare di aprire il paragrafo con "I peer della rete peer-to-peer".
...
// [REVISIONE]: "168 miliardi di trilioni" è una resa potenzialmente fuorviante di "168 billion trillion" (1.68×10^23). In italiano "trilione" è ambiguo (scala lunga vs breve). Suggerito esplicitare l'ordine di grandezza: "circa 1,68 × 10^23 (168 miliardi di miliardi di miliardi)" o equivalente.
Se l'output della funzione di hash corrisponde a un modello stabilito dal protocollo, il miner vince la lotteria e il blocco viene accettato come valido. In caso contrario modifica leggermente il blocco candidato e riprova. Nel periodo di questa stesura, in media un miner deve provare circa 168 miliardi di trilioni di blocchi candidati, e altrettante volte deve eseguire la funzione di hash, prima di trovare la combinazione vincente.
...
// [REVISIONE]: imprecisione tecnica — l'originale dice "the data... is called proof of work (PoW)": la PoW è la prova/dato, non l'algoritmo. La traduzione "sono definiti algoritmo proof of work" confonde la prova con l'algoritmo che la produce. Rimuovere "algoritmo" o riformulare. Inoltre "prova di lavoro" è usata solo qui mentre nel resto dei capitoli si usa "proof of work" — verificare coerenza terminologica.
...
// [REVISIONE]: incoerenza — "miners" con la "s" del plurale inglese; nel resto del libro il plurale di prestiti come "miner" resta invariato in italiano ("i miner"). Uniformare.
I miners che partecipano a una mining pool impostano il proprio software per creare blocchi candidati che assegnano la ricompensa a un indirizzo della pool. Da lì, una quota della ricompensa viene distribuita ai membri della pool in proporzione al lavoro svolto.
...
// [REVISIONE]: presenza di caratteri zero-width (U+200B) all'inizio del paragrafo seguente e dopo "due nuovi blocchi." (subito prima di "Al contrario"). Stesso problema più sotto a inizio del paragrafo "Ogni blocco minato sopra a quello contenente la transazione di Alice...". Rimuovere i caratteri invisibili: possono creare problemi di rendering, ricerca testuale e copia-incolla.
...
// [REVISIONE]: blocco TIP non tradotto — il contenuto è ancora in inglese. Tradurre il testo (es. "Puoi visualizzare il blocco che contiene la transazione di Alice."), lasciando invariato l'URL.
[TIP]
====
