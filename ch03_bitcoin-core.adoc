...
Le persone accettano denaro in cambio dei loro beni e servizi solo se credono che potranno spendere quel denaro successivamente. Denaro contraffatto o svalutato in maniera inattesa potrebbe non essere speso in futuro, perciò chiunque riceva bitcoin ha un forte incentivo a controllare attentamente l'integrità dei bitcoin che riceve.
// [REVISIONE]: "full verification node" tradotto come "nodo di verifica completa" — "full" modifica "node" (non "verification"), quindi il nodo è completo, non la verifica; preferire "nodo di verifica completo" o "nodo di validazione completo". Più avanti nella stessa frase la traduzione usa "nodi completi (full nodes)" correttamente, ma l'epiteto iniziale crea ambiguità.
// [REVISIONE]: omesso "entirely" ("interamente") dell'originale "software running entirely on your local computer"; l'avverbio sottolinea che il nodo di verifica gira completamente in locale, senza dipendere da terze parti — punto centrale della tesi sull'assenza di fiducia. Suggerito: "il software in esecuzione interamente sul tuo computer locale".
Il sistema Bitcoin è stato progettato in modo che sia possibile per il software in esecuzione sul tuo computer locale prevenire perfettamente la contraffazione, la svalutazione e diversi altri problemi critici. Il software che fornisce questa funzione è chiamato _nodo di verifica completa_, perché verifica ogni transazione Bitcoin confermata rispetto a tutte le regole del sistema. I nodi di verifica completa, o semplicemente _nodi completi_ (full nodes), possono anche fornire strumenti e dati utili per comprendere il funzionamento di Bitcoin e cosa sta succedendo attualmente nella rete.
...
// [REVISIONE]: "almost full time" reso come "a tempo pieno" — omesso "almost" (quasi); l'originale indica che la dozzina di sviluppatori lavora _quasi_ a tempo pieno, non a tempo pieno. Suggerito: "con circa una dozzina di sviluppatori impegnati quasi a tempo pieno".
...
Anche se Bitcoin Core rappresenta l'implementazione di riferimento per molte parti fondamentali del sistema, il whitepaper di Bitcoin descrive diverse delle prime componenti del sistema.
La maggior parte dei componenti principali dal 2011 in poi è documentata nei https://oreil.ly/BCXAQ[Bitcoin Improvement Proposals (BIPs)]. Nel corso di questo libro, ci riferiremo alle specifiche BIP utilizzando il loro numero; ad esempio, il BIP9 descrive un meccanismo utilizzato per diversi aggiornamenti importanti di Bitcoin.
...
// [REVISIONE]: "download the compressed archive of the source code" è reso come "scarica il file zip del codice sorgente" — l'originale dice "compressed archive" (archivio compresso, generico), non specificamente "zip"; Bitcoin Core distribuisce sia .zip sia .tar.gz. Preferire "scarica l'archivio compresso del codice sorgente".
Il codice sorgente di Bitcoin Core può essere scaricato come file zip o clonando il repository GitHub. Vai alla https://oreil.ly/hN9g1[pagina di download di Bitcoin Core], seleziona la versione più recente e scarica il file zip del codice sorgente. In alternativa, utilizza il comando Git per creare una copia locale del codice sorgente dal https://oreil.ly/BdOwl[repository GitHub di Bitcoin].
...
Al termine dell’operazione di clonazione eseguita da Git, avrai una copia locale completa del repository del codice sorgente nella cartella _bitcoin_. Passa a questa cartella con il comando +cd+:
...
// [REVISIONE]: "a specific snapshot of the code repository identified by a keyword tag" reso come "una versione precisa del codice salvata nel repository, identificata da un'etichetta": "snapshot" (istantanea del repository in un dato momento, termine tecnico Git) è parafrasato come "versione precisa salvata", perdendo il significato tecnico; "keyword tag" perde "keyword" (parola chiave). Suggerito: "uno snapshot preciso del repository identificato da un tag (parola chiave)".
...
L'elenco dei tag mostra tutte le versioni di Bitcoin che sono state rilasciate. Per convenzione, le versioni _candidate al rilascio_, destinate ai test, hanno il suffisso "rc". Le versioni stabili, adatte all'uso in ambienti di produzione, non hanno alcun suffisso.
// [REVISIONE]: "la versione più recente" rende "the highest version release" — il criterio dell'originale è il numero di versione più alto, non la cronologia. Nell'elenco dei tag (che include suffissi "rc") "più recente" può divergere da "numero più alto" ed essere fuorviante. Suggerito: "la versione con il numero più alto" (versione stabile).
...
// [REVISIONE]: "Review the main documentation" reso come "Ti consigliamo di consultare" — l'originale usa l'imperativo diretto (rivolgendosi a "you"), coerente con il registro TU del libro; la traduzione introduce una prima persona plurale implicita ("Ti consigliamo") non presente nell'originale. Suggerito: "Consulta la documentazione principale contenuta in _README.md_ nella directory _bitcoin_."
...
// [REVISIONE]: "Per istruzioni dettagliate sulla compilazione" aggiunge l'aggettivo "dettagliate" assente nell'originale ("Review the instructions for compiling the +bitcoind+ command-line client"); l'aggiunta caratterizza le istruzioni e crea un'aspettativa sul contenuto di build-unix.md non presente nell'EN. Suggerito: "Consulta le istruzioni per compilare il client da riga di comando +bitcoind+...".
In questo capitolo, compileremo il daemon di Bitcoin Core (server), noto come +bitcoind+ su Linux (un sistema Unix-like). Per istruzioni dettagliate sulla compilazione del client da riga di comando +bitcoind+ per la tua piattaforma, consulta il file _doc/build-unix.md_.
Istruzioni alternative si possono trovare nella directory _doc_; ad esempio, _build-windows.md_ per le istruzioni relative a Windows. Al momento della stesura di questo testo, sono disponibili istruzioni per Android, FreeBSD, NetBSD, OpenBSD, macOS (OSX), Unix e Windows.

Esamina attentamente i prerequisiti per la compilazione, che si trovano nella prima parte della documentazione sulla build. Si tratta di librerie che devono essere presenti sul tuo sistema prima di iniziare a compilare Bitcoin.
...
Lo script _autogen.sh_ genera una serie di script di configurazione automatica che interrogheranno il tuo sistema per individuare le impostazioni corrette e verificare che siano presenti tutte le librerie necessarie per compilare il codice.
...
// [REVISIONE]: "without any of the optional features" reso come "senza utilizzare flag di configurazione" (significato diverso: l'originale dice che si compila senza funzionalità opzionali, non senza flag); omesse inoltre le frasi EN "possibly enable features that you want" e "We recommend looking at each option to see if it's relevant", che invitano il lettore a rivedere e abilitare le opzioni desiderate.
...
Se ti trovi in un ambiente accademico, le restrizioni del laboratorio informatico potrebbero richiederti di installare le applicazioni nella tua directory personale (ad esempio, utilizzando l'opzione +--prefix=$HOME+).
...
<dd><p>Questo sovrascrive la posizione di installazione predefinita (che è <em>/usr/local/</em>) per l'eseguibile risultante. Usa <code>$HOME</code> per inserire tutto nella tua directory home, o un percorso diverso.</p></dd>

<dt><code>--disable-wallet</code></dt>
<dd><p>Questo serve per disabilitare l'implementazione del wallet di riferimento.
</p></dd>

<dt><code>--with-incompatible-bdb</code></dt>
<dd><p>Se stai compilando un wallet, consenti l'uso di una versione incompatibile della libreria Berkeley DB.
</p></dd>

<dt><code>--with-gui=no</code></dt>
<dd><p>Non compilare l'interfaccia utente grafica, che richiede la libreria Qt. Questo compila soltanto il server e la versione a riga di comando di Bitcoin Core.</p></dd>
</dl>
++++
...
Se è andato tutto bene, il comando +configure+ terminerà creando gli script di build personalizzati che ci permetteranno di compilare +bitcoind+. Se invece mancano librerie o si verificano errori, il comando +configure+ si interromperà con un errore anziché generare gli script di build.
Se si verifica un errore, è molto probabile che sia dovuto alla mancanza di una libreria necessaria o ad una libreria incompatibile. Rivedi la documentazione relativa alla compilazione e assicurati di installare tutti i prerequisiti mancanti. Dopodiché, esegui nuovamente +configure+ e verifica se il problema è stato risolto.
...
Se tutto è andato bene, Bitcoin Core è ora compilato. A questo punto, dovresti eseguire la suite di unit test con il comando +make check+ per verificare che le librerie collegate non presentino errori evidenti.
...
// [REVISIONE]: "help make it more robust" è reso come "rendendola più robusta e sicura" (e analogamente "rendere la rete più robusta e sicura" nel punto elenco di questa stessa sezione); l'originale cita solo "more robust", non menziona la sicurezza. Suggerito: "rendendola più robusta" e nel punto elenco "rendere la rete più robusta".
...
// [REVISIONE]: "has a low data cap" reso come "ha un tetto massimo di dati": si omette "low" (basso). L'originale individua come problematiche le connessioni con un limite dati _basso_, non genericamente quelle con un tetto dati (qualsiasi connessione a consumo ha un massimo). L'omissione amplia indebitamente la raccomandazione. Suggerito: "ha un limite di dati basso".
...
// [REVISIONE]: "need to rely on a Bitcoin node for programmable (API) access" — "programmable" (accesso programmabile/via codice) è omesso nella traduzione; preferire "per accedere alla rete e alla blockchain in modo programmabile (tramite API)".
...
++conf++:: Una posizione alternativa per il file di configurazione. Questa opzione ha senso solo come parametro da riga di comando per +bitcoind+, poiché non può trovarsi all'interno del file di configurazione a cui si riferisce.

// [REVISIONE]: "the maximum size expected to increase by several hundred gigabytes per year" reso come "con una crescita prevista di diverse centinaia di gigabyte all'anno": si omette "maximum size" (dimensione massima), indicazione tecnica rilevante per la pianificazione dei requisiti hardware a lungo termine. Suggerito: "con la dimensione massima prevista in crescita di diverse centinaia di gigabyte all'anno".
++datadir++:: Seleziona la directory e il filesystem in cui salvare tutti i dati della blockchain. Per impostazione predefinita, viene utilizzata la sottodirectory _.bitcoin_ nella tua home directory. A seconda della tua configurazione, lo spazio utilizzato può variare da circa 10 GB fino a quasi 1 TB al momento della scrittura, con una crescita prevista di diverse centinaia di gigabyte all'anno.
...
// [REVISIONE]: "This allows you to programmatically retrieve any transaction" è reso come "Questo consente di recuperare tramite script qualsiasi transazione": "tramite script" restringe il significato a script/procedure automatizzate, mentre "programmatically" indica genericamente qualsiasi accesso via codice. Suggerito: "recuperare in modo programmatico" o "recuperare tramite codice".
...
// [REVISIONE]: "instead of relaying unconfirmed transactions" — "relay" ha un significato tecnico specifico (propagazione P2P di transazioni ai peer); "trasmettere" è più generico. Preferire "invece di fare il relay di transazioni non confermate" o "invece di propagare transazioni non confermate ai peer".
...
// [REVISIONE]: nel blocco sidebar seguente, "_any_ transaction" nell'originale è in corsivo enfatico; la traduzione usa "qualsiasi transazione" senza enfasi, perdendo il contrasto con il comportamento predefinito (solo transazioni del wallet). Preferire "_qualsiasi_ transazione" per preservare l'enfasi tecnica dell'originale.
[[txindex]]
...
// [REVISIONE]: "running as an API backend for a bitcoin application" reso come "eseguendolo come un'API backend": calco con testa del sintagma invertita — il nodo funge da _backend_ (con interfaccia API), non da "API"; in italiano "un'API backend" è fuorviante. Suggerito: "eseguendolo come backend API per un'applicazione Bitcoin".
...
.Configurazione di esempio per un sistema a risorse limitate.
====
----
alertnotify=myemailscript.sh "Alert: %s"
blocksonly=1
...
Questo mostra un nodo con un'altezza della blockchain pari a 0 blocchi e 83.999 header. Il nodo, per prima cosa, scarica gli header dei blocchi dai suoi peer per individuare la blockchain con la maggiore proof of work e, successivamente, procede a scaricare i blocchi completi, validandoli man mano.

Una volta soddisfatto delle opzioni di configurazione selezionate, dovresti aggiungere Bitcoin Core agli script di avvio del tuo sistema operativo, in modo che rimanga in esecuzione continuamente e si riavvii al riavvio del sistema operativo. Troverai diversi esempi di script di avvio per vari sistemi operativi nella directory dei sorgenti di Bitcoin Core sotto _contrib/init_ e un file _README.md_ che indica quale script è utilizzato da ciascun sistema.

=== API Bitcoin Core 

// [REVISIONE]: "available programmatically via the API" reso come "disponibili anche a livello di programmazione tramite le API": l'originale usa "the API" (singolare, riferito all'unica interfaccia JSON-RPC di Bitcoin Core); "le API" (plurale) è impreciso nel contesto. Suggerito: "disponibili anche programmaticamente tramite l'API".
// [REVISIONE]: "command-line helper" reso qui come "strumento a riga di comando", mentre altrove nel capitolo lo stesso termine "helper" è reso coerentemente con "l'helper +bitcoin-cli+" (es. righe successive: "l'helper +bitcoin-cli+ o il client HTTP +curl+"). Uniformare la terminologia, es. "l'helper a riga di comando +bitcoin-cli+".
Bitcoin Core implementa un'interfaccia JSON-RPC a cui si può accedere anche utilizzando lo strumento a riga di comando +bitcoin-cli+. La riga di comando ci permette di sperimentare in modo interattivo le funzionalità che sono disponibili anche a livello di programmazione tramite le API. Per iniziare, esegui il comando +help+ per vedere un elenco dei comandi RPC di Bitcoin Core disponibili:
...
Ognuno di questi comandi può accettare diversi parametri. Per ottenere ulteriore aiuto, una descrizione dettagliata e le informazioni sui parametri, aggiungi il nome del comando dopo +help+. Ad esempio, per visualizzare l'aiuto relativo al comando RPC +getblockhash+:
...
// [REVISIONE]: "two examples of the RPC command" (singolare, riferito all'unico comando getblockhash appena illustrato) reso come "due esempi di comandi RPC" (plurale generico): si perde il riferimento all'unico comando in esame, facendo intendere che siano esempi di comandi RPC diversi. Suggerito: "due esempi del comando RPC" o "due esempi di invocazione del comando RPC".
...
// [REVISIONE]: l'originale usa "include" ("The most important commands include..."), che introduce un elenco non esaustivo; la resa "I comandi più importanti sono..." presenta l'elenco come chiuso/esaustivo, restringendo l'informazione tecnica. Suggerito: "Tra i comandi più importanti ci sono..." o "I comandi più importanti includono...".
Bitcoin Core fornisce rapporti di stato sui diversi moduli tramite l'interfaccia JSON-RPC. I comandi più importanti sono +getblockchaininfo+, +getmempoolinfo+, +getnetworkinfo+ e +getwalletinfo+.
...
// [REVISIONE]: "also quite human-readable" tradotto come "anche piuttosto intuitivo per noi esseri umani" — "human-readable" è un termine tecnico standard (leggibile dall'uomo senza strumenti di decodifica), non equivalente a "intuitivo". Preferire "leggibile anche dall'essere umano" o "comprensibile anche senza strumenti di decodifica".
...
// [REVISIONE]: nel secondo periodo "mettendo l'ID della transazione (txid) come parametro" rende "by passing ... as a parameter" con una collocazione poco idiomatica; il verbo tecnico corretto è "passare" (passare un valore come parametro), anche per coerenza con "passando i dati esadecimali come parametro" usato poco più avanti nella stessa sezione
...
[TIP]
====
// [REVISIONE]: "non sono autorevoli" non rende il senso tecnico di "not authoritative" in questo contesto: il txid non è un identificatore definitivo/certo finché la transazione non è confermata in un blocco; preferire "non sono identificatori certi/definitivi" o "non costituiscono prova definitiva"
...
// [REVISIONE]: "its legacy stripped size" è reso come "la stripped size (misurazione legacy, ottenuta rimuovendo i dati witness)": la glossa "ottenuta rimuovendo i dati witness" non è presente nell'originale ed è un'aggiunta editoriale. Suggerito: rimuovere la glossa oppure segnalarla come nota del traduttore.
La voce +confirmations+ ci indica la _profondità_ (depth) di questo blocco--quanti blocchi sono stati costruiti sopra di esso, indicando la difficoltà di modificare qualsiasi transazione in questo blocco. La +height+ (altezza) ci dice quanti blocchi hanno preceduto questo blocco. Vediamo la versione del blocco, l'ora in cui è stato creato (secondo il suo miner), il tempo mediano degli 11 blocchi che precedono questo blocco (una misurazione temporale più difficile da manipolare per i miner), e la dimensione del blocco in tre diverse misurazioni: la stripped size (misurazione legacy, ottenuta rimuovendo i dati witness), la full size (dimensione completa) e le weight units (dimensione in unità di peso). Vediamo anche alcuni campi utilizzati per la sicurezza e la proof of work (merkle root, nonce, bits, difficulty e chainwork); esamineremo questi in dettaglio in <<mining>>.

==== Utilizzo dell'interfaccia programmabile di Bitcoin Core
// [REVISIONE]: "to access functions programmatically" reso come "permettere l'accesso alle funzionalità in modo programmabile" — "functions" (funzioni/metodi richiamabili dell'API) diventa "funzionalità" (capacità/caratteristiche generiche); e "programmatically" (tramite codice/a livello di programmazione) diventa "in modo programmabile" (che può essere configurato/programmato), con uno slittamento semantico. Suggerito: "accedere alle funzioni [dell'API] tramite codice".
...
RPC sta per remote [.keep-together]#procedure# call (chiamata di procedura remota), il che significa che stiamo invocando procedure (funzioni) che si trovano in remoto (sul nodo Bitcoin Core) tramite un protocollo di rete. In questo caso, il protocollo di rete utilizzato è HTTP.
...
// [REVISIONE]: "se fornito con la directory dei dati" è un calco di "given the data directory" e risulta ambiguo (il soggetto sottinteso di "fornito" non è chiaro); il senso è che l'helper legge il file solo se gli viene indicata la directory dei dati: preferire "se gli viene fornita/indicata la directory dei dati"
...
[[cookie_auth]]
.Uso dell'autenticazione cookie-based con Bitcoin Core
====
----
$ cat .bitcoin/.cookie
...
// [REVISIONE]: "libraries that 'wrap' the Bitcoin Core API" — "wrap" (virgolettato nell'originale) è tradotto come "incapsulano": in programmazione "wrap" (creare un wrapper) e "encapsulate" (incapsulamento OOP) hanno significati tecnici distinti. Preferire "che forniscono un'interfaccia semplificata per l'API di Bitcoin Core" o "che fanno il wrap dell'API di Bitcoin Core".
Tuttavia, esistono librerie nella maggior parte dei linguaggi di programmazione più diffusi che incapsulano l'API di Bitcoin Core rendendo questo processo molto più semplice.
...
// [REVISIONE]: "this requires you to have a running Bitcoin Core instance" reso come "è necessario avere un’istanza attiva di Bitcoin Core" — l’originale usa la seconda persona diretta ("you"), coerente con il registro TU del libro; la traduzione usa la forma impersonale. Suggerito: "Ricorda che devi avere un’istanza attiva di Bitcoin Core, che verrà usata per effettuare chiamate JSON-RPC."
...
// [REVISIONE]: "+getrawtransaction+ e +decodetransaction+" — il nome del secondo comando è errato: il comando bitcoin-cli corretto è +decoderawtransaction+ (correttamente citato poco prima nel capitolo nella sezione "Esplorare e Decodificare le Transazioni"). "+decodetransaction+" non esiste nell'API di Bitcoin Core. Suggerito: "+getrawtransaction+ e +decoderawtransaction+".
...
[[rpc_transaction]]
.Recupero di una transazione e iterazione dei suoi output
====
[source,python]
----
...
[[rpc_block]]
.Recupero di un blocco e somma di tutti gli output delle transazioni
====
[source,python]
----
...
// [REVISIONE]: (1) "full-node implementations" tradotto come "implementazioni complete di full node" — "full-node" è aggettivo composto che descrive il tipo di nodo, non un attributo di completezza; preferire "implementazioni di full node"; (2) "native interfaces" (plurale) tradotto come "un’interfaccia nativa" (singolare) — ogni libreria per linguaggio diverso ha le sue interfacce native; preferire "interfacce native nel linguaggio che preferiscono".
...
==== C/C++
// [REVISIONE]: "The reference implementation of Bitcoin" reso come "La versione di riferimento di Bitcoin": "implementation" (implementazione) ≠ "versione" (version); il termine "implementazione di riferimento" è già usato correttamente nel capitolo. Suggerito: "L'implementazione di riferimento di Bitcoin".
...
==== Python
// [REVISIONE]: "un full node" aggiunge il qualificatore "full" assente nell'originale ("consensus library, and node by Peter Todd", cioè un "node" generico); in Bitcoin un full node verifica tutte le regole di consenso mentre "node" non lo implica. Suggerito: "un nodo". Da notare l'incoerenza con la voce "Go" che rende "full-node" con "full node".
...
==== Rust
// [REVISIONE]: "parsing" (analisi sintattica di strutture dati binarie Bitcoin) reso come "l'analisi dei dati": "analisi dei dati" è un concetto più ampio e non equivalente al termine tecnico "parsing". Suggerito: conservare "parsing" o usare "analisi sintattica".
...
==== Scala
// [REVISIONE]: nell'elenco delle librerie, la voce "bitcoin-s" aggiunge enfasi corsivo "_implementazione_" non presente nell'originale ("A Bitcoin implementation in Scala"); nessun'altra voce dell'elenco usa enfasi su "implementation". Incoerenza editoriale interna all'elenco. Suggerito: "Un'implementazione Bitcoin in Scala".
https://bitcoin-s.org[bitcoin-s]:: Un'_implementazione_ Bitcoin in Scala
...
// [REVISIONE]: "rendere l’invio e la ricezione delle transazioni più convenienti" aggiunge "delle transazioni" (non nell’originale "spending and receiving") e "più" (l’originale dice "convenient", non "more convenient") — preferire "rendere l’invio e la ricezione [di bitcoin] più agevoli" o semplicemente "semplici"
// [REVISIONE]: "without having to trust any outside authority" — "trust" ha il significato tecnico e filosofico preciso nel contesto Bitcoin (concetto di trustless); "fare affidamento su" descrive dipendenza funzionale, non fiducia in senso tecnico. Preferire "senza dover riporre fiducia in alcuna autorità esterna".
// [REVISIONE]: "any bitcoins you receive" reso come "i bitcoin che ricevi": l'EN usa "any" per indicare universalità (qualsiasi bitcoin ricevuto), mentre l'articolo determinativo "i bitcoin" può riferirsi a bitcoin specifici, perdendo la generalizzazione tecnica. Suggerito: "qualsiasi bitcoin ricevuto".
