...
// [REVISIONE]: "controllare attentamente l'integrità" è impreciso rispetto all'originale "verify the integrity" — preferire "verificare l'integrità"
...
// [REVISIONE]: "altri problemi critici" omette "several" dell'originale ("several other critical problems") — preferire "diversi altri problemi critici"
...
// [REVISIONE]: "analizzeremo" non rende bene "inspect" dell'originale, che indica un'ispezione diretta dei dati del proprio nodo; preferire "esamineremo" o "ispezioneremmo"
...
// [REVISIONE]: l'originale usa "BIPs" (plurale con la s), che è la forma standard corretta della sigla. "Bitcoin Improvement Proposals (BIP)" omette la s del plurale — preferire "Bitcoin Improvement Proposals (BIPs)"
La maggior parte dei componenti principali dal 2011 in poi è documentata nei https://oreil.ly/BCXAQ[Bitcoin Improvement Proposals (BIP)]. Nel corso di questo libro, ci riferiremo alle specifiche BIP utilizzando il loro numero; ad esempio, il BIP9 descrive un meccanismo utilizzato per diversi aggiornamenti importanti di Bitcoin.
...
// [REVISIONE]: l'originale dice "compressed archive" (archivio compresso), non "file zip" specificamente — preferire "archivio compresso" per entrambe le occorrenze in questo paragrafo
...
// [REVISIONE]: "la versione più recente" non rende "the highest version release" dell'originale — l'inglese indica la versione con il numero più alto tra le release stabili, non necessariamente la più recente cronologicamente; preferire "la versione con il numero più alto" o "la versione stabile più recente"
...
// [REVISIONE]: "Linux (o altri sistemi Unix)" non rende "Linux (a Unix-like system)" — la parentesi originale descrive Linux come sistema Unix-like, non indica altri sistemi; preferire "Linux (un sistema Unix-like)"
...
// [REVISIONE]: "potresti dover installare" omette la motivazione esplicita dell'originale: "computer lab restrictions may require you to install" — le restrizioni del laboratorio informatico impongono l'installazione nella home directory; preferire "le restrizioni del laboratorio informatico potrebbero imporre di installare"
...
// [REVISIONE]: "rendendola più robusta e sicura" aggiunge "e sicura" non presente nell'originale "help make it more robust" — rimuovere "e sicura"
...
// [REVISIONE]: "più robusta e sicura" aggiunge "e sicura" non presente nell'originale "makes the network more robust" — rimuovere "e sicura"
...
// [REVISIONE]: l'originale usa "Ctrl-C" (con trattino); più avanti nel file viene usato correttamente "Ctrl-C" — uniformare a "Ctrl-C" per coerenza con l'originale e con il resto del file
...
// [REVISIONE]: "recuperare tramite script" è impreciso rispetto a "programmatically retrieve" dell'originale — preferire "recuperare in modo programmatico" o "recuperare via programma"
...
// [REVISIONE]: "trasmettere" non rende "relaying" — in contesto P2P Bitcoin, "relay" indica l'inoltro/ritrasmissione di transazioni ai peer; preferire "invece di inoltrare/ritrasmettere transazioni non confermate"
...
// [REVISIONE]: "un API" è scorretto grammaticalmente (API inizia con vocale) — preferire "un'API backend"
...
// [REVISIONE]: "83.999 header" usa il punto come separatore delle migliaia (italiano), corretto; tuttavia più avanti a riga 536 si usa "775,072" con la virgola anglosassone — uniformare a "775.072"
...
// [REVISIONE — DA RIVEDERE]: l'originale inglese dice "Bitcoin's +getblockchaininfo+ RPC command was introduced earlier" (e non "Bitcoin Core's"); la traduzione "Il comando RPC +getblockchaininfo+ di Bitcoin" rispecchia fedelmente l'originale, ma sia in italiano che in inglese il riferimento è impreciso: gli RPC sono di Bitcoin Core (l'implementazione), non del protocollo Bitcoin. Valutare se correggere in "L'RPC +getblockchaininfo+ di Bitcoin Core" e segnalare l'imprecisione anche nell'originale O'Reilly.
...
// [REVISIONE]: "775,072" usa la virgola come separatore delle migliaia in stile anglosassone — in italiano il separatore delle migliaia è il punto: "775.072"
...
// [REVISIONE]: imprecisione tecnica nella resa di "its legacy stripped size". L'originale dice "the size of the block in three different measurements (its *legacy* stripped size, its full size, and its size in weight units)" — "legacy" qualifica "stripped size" (la stripped size è essa stessa una misurazione legacy/pre-SegWit, ottenuta rimuovendo i dati witness). La traduzione "stripped size (dimensione senza dati legacy)" capovolge il senso, lasciando intendere che la stripped size sia ottenuta rimuovendo i "dati legacy", quando in realtà si ottiene rimuovendo i dati *witness*. Suggerito: "la stripped size (misurazione legacy, senza dati witness)" oppure "la dimensione legacy (stripped size)".
...
// [REVISIONE — DA RIVEDERE]: calco "wrappano" — preferire una resa più naturale come "incapsulano", "fanno da wrapper a" o "fanno da interfaccia a" l'API di Bitcoin Core.
...
==== Python
// [REVISIONE]: "sviluppati" ha concordanza errata — il soggetto di pycoin è "Una libreria" (singolare femminile), quindi deve essere "sviluppata"; per python-bitcoinlib il participio regge una lista mista (libreria + full node), ma resta preferibile "sviluppata" accordandosi con il sostantivo principale
...
==== Scala
// [REVISIONE]: "Una versione di Bitcoin sviluppata in Scala" non rende "A Bitcoin implementation in Scala" — "versione" implica una variante del protocollo, mentre "implementation" indica un'implementazione software; preferire "Un'implementazione Bitcoin in Scala"
