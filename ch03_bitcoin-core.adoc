...
// [REVISIONE]: ridondanza "Vai alla pagina ... pagina di download di Bitcoin Core" — il testo del link contiene già "pagina". Riformulare ad es. "Vai alla https://oreil.ly/hN9g1[pagina di download di Bitcoin Core]". Stessa ridondanza nella seconda parte: "dalla pagina https://oreil.ly/BdOwl[GitHub di Bitcoin]" → "dal https://oreil.ly/BdOwl[repository GitHub di Bitcoin]".
...
// [REVISIONE]: refuso — "interroghrranno" deve diventare "interrogheranno".
Lo script _autogen.sh_ genera una serie di script di configurazione automatica che interroghrranno il tuo sistema per individuare le impostazioni corrette e verificare che siano presenti tutte le librerie necessarie per compilare il codice.
...
// [REVISIONE]: nel blocco passthrough HTML che segue, all'interno del <dd> dell'opzione "--with-gui=no", è presente un carattere zero-width (U+200B) immediatamente dopo "<p>" e prima di "Non compilare". Rimuoverlo per evitare problemi di rendering, ricerca testuale e copia-incolla.
++++
<dl>
...
[[full_index_node]]
// [REVISIONE]: refuso — "configutazione" deve diventare "configurazione".
...
// [REVISIONE]: punto finale anomalo nel titolo della sezione (l'originale "Getting Information on Bitcoin Core's Status" non ha punto); i titoli AsciiDoc non terminano in punto, valutare la rimozione.
==== Ottenere informazioni sullo stato di Bitcoin Core.

// [REVISIONE]: congiunzione "and" rimasta in inglese — tradurre con "e" (es. "+getnetworkinfo+ e +getwalletinfo+").
...
// [REVISIONE]: l'originale dice "The Bitcoin Core RPC +getblockchaininfo+..."; la traduzione "Il comando RPC +getblockchaininfo+ di Bitcoin" omette "Core" — Bitcoin (il sistema/protocollo) e Bitcoin Core (l'implementazione che fornisce gli RPC) non vanno confusi. Ripristinare "L'RPC +getblockchaininfo+ di Bitcoin Core" o equivalente.
...
[[alice_tx_serialized]]
// [REVISIONE]: didascalia non tradotta — "Alice's serialized transaction" andrebbe resa in italiano (es. "Transazione serializzata di Alice"), in coerenza con le altre didascalie del capitolo.
...
// [REVISIONE]: errore di markup nel testo italiano — l'originale chiude con "the network protocol is HTTP." (senza footnote). Nella traduzione "il protocollo di rete utilizzato è _HTTP.5_ ." sono presenti: (a) un ".5" spurio (probabile residuo di un vecchio footnote marker numerato non più presente nell'originale corrente); (b) underscore che applicano erroneamente il corsivo a "HTTP.5"; (c) uno spazio prima di un punto finale duplicato. Sostituire con "il protocollo di rete utilizzato è HTTP." senza alcuna footnote.
...
[[cookie_auth]]
// [REVISIONE]: didascalia non tradotta — "Using cookie-based authentication with Bitcoin Core" da rendere in italiano (es. "Uso dell'autenticazione cookie-based con Bitcoin Core") per coerenza con le altre didascalie tradotte.
...
// [REVISIONE]: calco "wrappano" — preferire una resa più naturale come "incapsulano", "fanno da wrapper a" o "fanno da interfaccia a" l'API di Bitcoin Core.
...
[[rpc_example]]
// [REVISIONE]: didascalia non tradotta — "Running +getblockchaininfo+ via Bitcoin Core's JSON-RPC API" da rendere in italiano (es. "Esecuzione di +getblockchaininfo+ tramite l'API JSON-RPC di Bitcoin Core").
...
[[rpc_transaction]]
// [REVISIONE]: didascalia non tradotta — "Retrieving a transaction and iterating its outputs" da rendere in italiano (es. "Recupero di una transazione e iterazione dei suoi output").
...
[[rpc_block]]
// [REVISIONE]: didascalia non tradotta — "Retrieving a block and adding all the transaction outputs" da rendere in italiano (es. "Recupero di un blocco e somma di tutti gli output delle transazioni").
...
// [REVISIONE]: parola non tradotta — `[.keep-together]#convenient.#` lascia "convenient" in inglese; tradurre (es. "convenienti." o "comode.") mantenendo il markup `[.keep-together]#...#` e correggendo il punto finale duplicato.
