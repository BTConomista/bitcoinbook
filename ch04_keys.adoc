...
Il destinatario, come Bob, riceve bitcoin su una chiave pubblica all'interno di una transazione firmata dal mittente (come Alice). I bitcoin che Alice sta spendendo erano stati precedentemente ricevuti su una delle sue chiavi pubbliche, e lei utilizza la corrispondente chiave privata per generare la sua firma.
...
La crittografia a chiave pubblica((("public key cryptography", id="pub-key"))) è stata inventata negli anni '70 ed è la base matematica della sicurezza informatica moderna.
Da allora, sono state scoperte diverse funzioni matematiche adatte, come l’elevazione a potenza di numeri primi e la moltiplicazione scalare su curva ellittica. Queste funzioni matematiche sono facili da calcolare in una direzione ma praticamente impossibili da invertire con i computer e gli algoritmi attualmente disponibili.
...
In Bitcoin, possiamo usare la crittografia a chiave pubblica per creare((("key pairs", id="key-pair")))((("public keys", "purpose of")))((("private keys", "purpose of"))) una coppia di chiavi che controlla l’accesso ai bitcoin. La coppia di chiavi è composta da una chiave privata e una chiave pubblica derivata dalla chiave privata. La chiave pubblica viene utilizzata per ricevere fondi, mentre la chiave privata serve a firmare le transazioni per spendere quei fondi.
...
****
La crittografia asimmetrica((("asymmetric cryptography", see="public key cryptography")))((("public key cryptography", "purpose in Bitcoin")))((("digital signatures"))) in Bitcoin non viene utilizzata per "cifrare" (rendere segrete) le transazioni. Piuttosto, si usa una delle proprietà più utili della crittografia asimmetrica, cioè la possibilità di generare _firme digitali_.
...
[[chiavi_private]]
==== 4.2 Chiavi private
Una chiave((("private keys", "generating", id="private-key-generate"))) privata è semplicemente un numero scelto in modo casuale. Il controllo della chiave privata è la base del controllo dell'utente su tutti i fondi associati alla corrispondente chiave pubblica di Bitcoin.
...
Il primo e più importante passaggio nella generazione delle chiavi è trovare una fonte di casualità affidabile (in informatica, detta ((("entropy")))_entropia_). Creare una chiave Bitcoin è praticamente come “scegliere un numero tra 1 e 2^256.” Non ha importanza il metodo specifico con cui scegli questo numero, purché non sia prevedibile né ripetibile. Il software Bitcoin utilizza generatori di numeri casuali crittograficamente sicuri per produrre 256 bit di entropia.
...
La crittografia a curva ellittica (ECC) è((("public key cryptography", "elliptic curve cryptography as", id="pub-key-ecc")))((("elliptic curve cryptography (ECC)", id="ecc"))) un tipo di crittografia asimmetrica, o crittografia a chiave pubblica, basata sul problema del logaritmo discreto applicato all’addizione e alla moltiplicazione dei punti su una curva ellittica.
