[[ch04_keys_addresses]]
== 4. Chiavi e indirizzi

// [REVISIONE]: la traduzione amplia la portata dell'originale. L'EN lega il vincolo di privacy solo a "Bob's real-world identity" e a "other Bitcoin payments that Bob receives"; l'IT introduce anche l'identità di Alice ("alla sua identità reale") e "altri pagamenti Bitcoin che loro inviano o ricevono" (verbo "inviano" e soggetto plurale assenti nell'originale). Verificare aderenza all'EN: "...all'identità reale di Bob o ad altri pagamenti Bitcoin che Bob riceve."
Alice vuole pagare Bob. Le migliaia di full nodes Bitcoin che verificheranno la sua transazione non sanno chi sono Alice e Bob--e vogliamo che rimanga così per proteggere la loro privacy. Alice deve comunicare alla rete che Bob deve ricevere alcuni dei suoi bitcoin senza collegare alcun aspetto di quella transazione alla sua identità reale o a quella di Bob, o ad altri pagamenti Bitcoin che loro inviano o ricevono. Il metodo che Alice utilizza deve garantire che solo Bob possa spendere i bitcoin che riceve.

Il whitepaper originale di Bitcoin descrive un sistema molto semplice per raggiungere questi obiettivi, mostrato in figura 4.1 <<pay-to-pure-pubkey>>

[[pay-to-pure-pubkey]]
.Catena di Transazioni dal whitepaper di Satoshi Nakamoto.
image::images/mbc3_aain01.png["Transaction chain from original Bitcoin paper"]

// [REVISIONE]: nella frase "I full node possono verificare...", "commits to" (impegno/vincolo crittografico, commitment) compare due volte nell'originale ma è reso in modo incoerente: "si riferisce a" (troppo debole, perde il senso di commitment) e poi "lega". Usare una resa coerente e tecnicamente fedele in entrambe le occorrenze, es. "vincola/impegna".
Il destinatario, in questo caso Bob, riceve bitcoin su una chiave pubblica all'interno di una transazione firmata dal mittente (Alice). I bitcoin che Alice sta spendendo erano stati precedentemente ricevuti su una delle sue chiavi pubbliche, e ora lei utilizza la corrispondente chiave privata per generare la sua firma.
I full node possono verificare che la firma di Alice si riferisca all'output di una funzione di hash, che a sua volta lega la transazione alla chiave pubblica di Bob e ad altri dettagli della transazione.

In questo capitolo esamineremo le chiavi pubbliche, le chiavi private, le firme e le funzioni di hash. Successivamente, utilizzeremo tutti questi elementi insieme per descrivere gli indirizzi utilizzati dal software Bitcoin moderno.

=== 4.1 Crittografia a Chiave Pubblica

La crittografia a chiave pubblica è stata inventata negli anni '70 ed è la base matematica della sicurezza informatica moderna.
Da allora, sono state scoperte diverse funzioni matematiche adatte a creare sistemi crittografici sicuri, come l’elevazione a potenza di numeri primi e la moltiplicazione scalare su curva ellittica. Queste funzioni matematiche sono facili da calcolare in una direzione ma praticamente impossibili da invertire con i computer e gli algoritmi attualmente disponibili.
Sulla base di queste funzioni matematiche, la crittografia permette la creazione di firme digitali non falsificabili.
Bitcoin utilizza la somma di punti su curva ellittica e la moltiplicazione scalare su curva ellittica come base per la sua crittografia.

Nel protocollo Bitcoin, si utilizza la crittografia a chiave pubblica per generare una coppia di chiavi crittografiche che definisce chi può spendere determinati bitcoin.
La coppia di chiavi è composta da una chiave privata e una chiave pubblica derivata dalla chiave privata. La chiave pubblica viene utilizzata per ricevere fondi, mentre la chiave privata serve a firmare le transazioni per spendere quei fondi.

Esiste una relazione matematica tra la chiave privata e la chiave pubblica che permette di utilizzare la chiave privata per generare firme sui messaggi. Queste firme possono essere verificate utilizzando la chiave pubblica, senza che la chiave privata venga rivelata.

[TIP]
====
Per comodità in alcuni wallet le chiavi private e quelle pubbliche sono conservate insieme come una _coppia di chiavi_.
Tuttavia, poiché la chiave pubblica può essere calcolata a partire dalla chiave privata, è anche possibile memorizzare solo la chiave privata senza bisogno di conservare esplicitamente la chiave pubblica.
====

Un wallet Bitcoin contiene una raccolta di coppie di chiavi, ognuna composta da una chiave privata e una chiave pubblica. La chiave privata (_k_) è un numero, solitamente derivato da un numero scelto casualmente. A partire dalla chiave privata, utilizziamo la moltiplicazione scalare su curva ellittica, una funzione crittografica unidirezionale, per generare la chiave pubblica (_K_).

[role="less_space pagebreak-before"]
.Perché usare la crittografia asimmetrica (chiavi pubbliche/privata)?
****
La crittografia asimmetrica in Bitcoin non viene utilizzata per "cifrare" (rendere segrete) le transazioni. Piuttosto, si usa una delle proprietà più utili della crittografia asimmetrica, cioè la possibilità di generare _firme digitali_.
Una chiave privata può essere applicata a una transazione per produrre una firma numerica. Questa firma può essere generata solo da chi possiede la chiave privata. Tuttavia, chiunque abbia accesso alla chiave pubblica e alla transazione può utilizzarle per _verificare_ la firma.
Questa proprietà della crittografia asimmetrica consente a chiunque di verificare ogni firma su ogni transazione, garantendo al contempo che solo i proprietari delle chiavi private possano generare firme valide.
****
...
[[chiavi_private]]
==== 4.2 Chiavi private
Una chiave privata è semplicemente un numero scelto in modo casuale. Il controllo della chiave privata è la base del controllo dell'utente su tutti i fondi associati alla corrispondente chiave pubblica.
La chiave privata viene utilizzata per creare firme digitali, che servono a spendere i bitcoin dimostrando il controllo dei fondi utilizzati in una transazione.
La chiave privata deve rimanere segreta in ogni momento, perché rivelarla a terzi equivale a dare loro il pieno controllo dei bitcoin protetti da quella chiave.
Inoltre, bisogna fare il backup della chiave privata ed essa deve essere protetta da eventuali perdite accidentali. Se una chiave privata viene smarrita, non può essere recuperata e i fondi ad essa associati saranno persi per sempre.

[TIP]
====
Una chiave privata Bitcoin è semplicemente un numero. Puoi scegliere la tua chiave privata in modo casuale usando solo una moneta, una matita e un foglio di carta: lancia una moneta 256 volte e otterrai le cifre binarie di una chiave privata casuale che puoi utilizzare in un wallet Bitcoin.
Da questa chiave privata, è poi possibile generare la chiave pubblica corrispondente.
Tuttavia, bisogna fare molta attenzione: qualsiasi processo che non sia completamente casuale può ridurre significativamente la sicurezza della chiave privata e, di conseguenza, dei bitcoin che essa protegge.
====

Il primo e più importante passaggio nella generazione delle chiavi è trovare una fonte di casualità affidabile (in informatica, detta _entropia_). Creare una chiave Bitcoin è praticamente come “scegliere un numero tra 1 e 2^256.” Non ha importanza il metodo specifico con cui scegli questo numero, purché non sia prevedibile né ripetibile. Il software Bitcoin utilizza generatori di numeri casuali crittograficamente sicuri per produrre 256 bit di entropia.

[role="less_space pagebreak-before"]
Più precisamente, la chiave privata può essere qualsiasi numero compreso tra 0 e _n_ - 1 (inclusi), dove _n_ è una costante (_n_ = 1,1578 × 10^77^, leggermente inferiore a 2^256^) definita come l’ordine della curva ellittica utilizzata in Bitcoin (vedi <<elliptic_curve>>). Per creare una chiave di questo tipo, scegliamo casualmente un numero a 256 bit e verifichiamo che sia inferiore a _n_.
In termini di programmazione, questo processo viene solitamente realizzato generando una stringa più lunga di bit casuali, raccolti da una fonte di casualità crittograficamente sicura, e passandola attraverso l’algoritmo di hash SHA256. Quest’ultimo produce comodamente un valore di 256 bit che può essere interpretato come un numero.
Se il risultato è inferiore a _n_, allora abbiamo generato una chiave privata valida. In caso contrario, semplicemente ripetiamo il processo con un altro numero casuale.

[WARNING]
====
Non usare codice personalizzato per generare numeri casuali, né utilizzare un 'semplice' generatore di numeri casuali offerto dal linguaggio di programmazione. Usa invece un generatore pseudocasuale crittograficamente sicuro (CSPRNG,"cryptographically secure pseudorandom number
generator"), inizializzato con un seed proveniente da una fonte con sufficiente entropia. Studia attentamente la documentazione della libreria che scegli per assicurarti che il generatore sia effettivamente sicuro dal punto di vista crittografico. Una corretta implementazione del CSPRNG è fondamentale per la sicurezza delle chiavi.
====

Di seguito è mostrata una chiave privata generata casualmente (_k_), rappresentata in formato esadecimale (256 bit visualizzati come 64 cifre esadecimali, ognuna delle quali rappresenta 4 bit):
...
[TIP]
====
Nella rete Bitcoin, il numero di chiavi private possibili è così enorme — 2^256^ — da risultare quasi impossibile da immaginare. In decimale è circa 10^77^. Tanto per capirci: in tutto l’universo visibile si stima che ci siano circa 10^80^ atomi.
====


[[elliptic_curve]]
==== La crittografia a Curve Ellittiche spiegata

La crittografia a curva ellittica (ECC) è un tipo di crittografia asimmetrica, o crittografia a chiave pubblica, basata sul problema del logaritmo discreto applicato all’addizione e alla moltiplicazione dei punti su una curva ellittica.

<<ecc-curve>> è un esempio di curva ellittica, simile a quella utilizzata da Bitcoin.
...
Bitcoin utilizza una specifica curva ellittica e un insieme di costanti matematiche, definiti in uno standard chiamato +secp256k1+, stabilito dal National Institute of Standards and Technology (NIST).
La curva +secp256k1+ è definita dalla seguente funzione, che genera una curva ellittica:
...
To work with both bech32 and bech32m, we'll look at the encoding and parsing rules for
