...
// [REVISIONE]: la traduzione "spingendo diversi sviluppatori di wallet a compiere scelte differenti" perde il termine tecnico chiave "trade-offs" dell'originale ("leading different wallet authors to make different trade-offs"), che indica compromessi consapevoli tra obiettivi in conflitto. Suggerito: "...portando diversi autori di wallet a operare compromessi differenti".
...
// [REVISIONE]: incoerenza terminologica — qui si usa "wallet hardware" mentre altrove nel capitolo e in ch01 si usa "dispositivo di firma hardware" (con nota esplicita che "hardware wallet" è fuorviante). Valutare "il dispositivo di firma hardware Trezor" per coerenza interna.
...
// [REVISIONE]: markup rotto — `(((che producono.` contiene un index marker AsciiDoc non chiuso (residuo di `((("key-stretching functions", startref="key-stretch")))they produce.` nell'originale). Rimuovere il `(((` orfano oppure ripristinare l'index marker chiuso.
...
// [REVISIONE]: refuso/grammatica — "le tue chiavi private generate chiavi sono selezionate" contiene un "chiavi" duplicato e usa l'indicativo dopo "assicurarti che" (richiede congiuntivo). L'originale: "you need to ensure your generated private keys are selected uniformly from within the entire 2^256^ range". Suggerito: "devi assicurarti che le tue chiavi private generate siano selezionate uniformemente dall'intero intervallo di 2^256^ chiavi private".
...
// [REVISIONE]: incoerenza terminologica interna al capitolo — qui (e a riga 906) si usa il calco "forzare brutalmente" / "forzare brutalmente l'altra metà", mentre poco sopra a righe 712 e 720 si usa "attacco a forza bruta" / "attaccare a forza bruta". Uniformare alla forma "attaccare a forza bruta" per coerenza terminologica e per evitare il calco da "brute-force".
...
// [REVISIONE]: calco dall'inglese "widely used" — "diffusamente utilizzato" suona innaturale in italiano. Suggerito: "ampiamente diffuso", "molto usato" o "il diffuso BTCPay Server open source".
