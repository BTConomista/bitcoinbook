...
[[ch05_wallets]]
// [REVISIONE]: Il titolo originale è "Wallet Recovery" — "Recupero del Wallet" è accettabile, ma nella terminologia del capitolo il tema principale è il backup e il recupero. Valutare "Recupero e Backup del Wallet" per riflettere meglio il contenuto.
== Recupero del Wallet
...
pratiche. Altre soluzioni comportano vantaggi e svantaggi, portando diversi sviluppatori di wallet a scendere a compromessi differenti.
...
// [REVISIONE]: "portafogli per contanti fisici" è una resa ibrida (portafogli per l'italiano, cash dall'inglese) e suona innaturale. L'originale dice "Wallets for physical cash" — suggerito: "I portafogli fisici contengono denaro contante" oppure "I portafogli tradizionali contengono denaro fisico".
...
// [REVISIONE]: L'originale dice "repeatable (deterministic) algorithm" — l'aggettivo "ripetibile" è omesso nella traduzione. Aggiungere: "...attraverso un algoritmo ripetibile (deterministico)." per preservare la sfumatura tecnica.
...
In <<public_key_derivation>>, abbiamo visto come creare una chiave pubblica da una chiave privata utilizzando la crittografia a curva ellittica (ECC). Sebbene le operazioni sulle curve ellittiche non siano intuitive, sono analoghe all’addizione, sottrazione e moltiplicazione dell’aritmetica tradizionale. In altre parole, è possibile sommare o sottrarre un valore a/da una chiave pubblica, oppure moltiplicarla. Considera l'operazione che abbiamo usato in <<public_key_derivation>> per generare una chiave pubblica (_K_) da una chiave privata (_k_) utilizzando il punto generatore (_G_):
...
// [REVISIONE]: La resa di "key tweak" come "(modifica applicata alla chiave)" è una parafrasi descrittiva aggiunta dal traduttore, non presente nell'originale. L'originale dice semplicemente "known as a _key tweak_". Valutare se mantenere il termine tecnico senza parafrasi, oppure usare una nota a piè di pagina, per non appesantire il testo.
...
// [REVISIONE]: "frontend" è usato come calco dall’inglese senza traduzione. L’originale usa "frontends" come termine tecnico accettato, ma nel testo italiano la parentesi esplicativa "(cioè la parte che interagisce con l’utente...)" suggerisce un’intenzione di chiarire il termine; valutare se mantenere "frontend" come tecnicismo oppure sostituire con "interfaccia utente" o "componente front-end" per coerenza con il livello di localizzazione scelto nel resto del libro.
...
I wallet HD sono un potente strumento per gestire numerose chiavi derivate da un unico seed. Se il tuo database wallet dovesse danneggiarsi o andare perso, puoi rigenerare tutte le chiavi private del tuo wallet utilizzando il seed originale. Tuttavia, se qualcun altro entra in possesso del tuo seed, può generare tutte le chiavi private e rubare tutti i bitcoin da un wallet a firma singola, oltre a ridurre la sicurezza dei bitcoin conservati in wallet multifirma.
In questa sezione esamineremo diversi _codici di recupero_, pensati per rendere i backup più semplici e sicuri.

// [REVISIONE]: "parole di uso comune" non cattura il senso di "human-language words" — l'originale enfatizza che si tratta di parole del linguaggio naturale/umano (contrapposto a stringhe esadecimali), non solo parole frequenti. Suggerito: "parole del linguaggio naturale" o "parole di una lingua naturale".
...
// [REVISIONE]: L'originale dice "for the past decade" — "negli ultimi dieci anni" è corretto, ma l'originale usa "decade" (= decennio). Da mantenere coerente con la nota tecnica: BIP39 è del 2013, non del 2014 come implica il testo della sezione successiva ("since 2014 or earlier").
BIP39::
...
// [REVISIONE]: "compleanno del wallet" è una traduzione letterale di "wallet birthday" che in italiano suona goffa. Il termine tecnico standard nelle traduzioni è "data di creazione del wallet" o "data di nascita del wallet". Considerare "data di creazione del wallet" (già usata in parte nella frase seguente).
...
// [REVISIONE]: "fermacampioni" è una resa errata di "brass fasteners" (graffette/fermagli in ottone usati per rilegare fogli). Suggerito: "fermagli in ottone" o "graffette in ottone".
...
// [REVISIONE]: La frase "A meno che tu non riesca a capire quale errore di battitura hai fatto in precedenza, i tuoi fondi sono persi" manca del contesto temporale dell'originale: "Unless you can figure out what typo you previously made, your funds are gone." — la traduzione è fedele, ma "i tuoi fondi sono persi" dovrebbe essere "i tuoi fondi sono perduti" o "i tuoi fondi sono andati" per preservare il tono definitivo dell'originale ("are gone"). Valutare anche "quei fondi sono persi per sempre".
...
// [REVISIONE]: "investigare" è un falso amico — in italiano significa indagare/fare un'indagine (accezione poliziesca), non "esaminare/approfondire". L'originale dice "investigate more modern standards" — suggerito: "esplorare", "approfondire" o "esaminare". Lo stesso problema si riscontra alle righe 265 e 290.
...
// [REVISIONE — DA RIVEDERE]: incoerenza terminologica — qui si usa "wallet hardware" mentre altrove nel capitolo e in ch01 si usa "dispositivo di firma hardware" (con nota esplicita che "hardware wallet" è fuorviante). Valutare "il dispositivo di firma hardware Trezor" per coerenza interna.
...
// [REVISIONE]: "tabella di lookup" è un calco dall'inglese "lookup table". Suggerito: "tabella di ricerca" o, se si vuole mantenere il tecnicismo, almeno "lookup table" in corsivo come termine straniero. La forma "tabella di lookup" è ibrida e incoerente.
...
siano selezionate uniformemente dall'intero intervallo di 2^256^ chiavi
private).

// [REVISIONE — DA RIVEDERE]: incoerenza terminologica interna al capitolo — qui (e poco più sotto) si usa il calco "forzare brutalmente" / "forzare brutalmente l'altra metà", mentre poco sopra si usa "attacco a forza bruta" / "attaccare a forza bruta". Uniformare alla forma "attaccare a forza bruta" per coerenza terminologica e per evitare il calco da "brute-force".
...
// [REVISIONE]: "wallet sotto coercizione" è una parafrasi di "duress wallet" — il termine tecnico inglese è usato in letteratura tecnica Bitcoin e potrebbe essere mantenuto tra virgolette come "duress wallet" con traduzione esplicativa, per permettere al lettore di riconoscerlo in altri contesti. La forma attuale è comprensibile ma non corrisponde al termine standard.
...
trovare i suoi fratelli a meno che non si abbia anche il codice di catena.
// [REVISIONE]: "codice di catena seed iniziale" è una resa letterale di "initial chain code seed" che in italiano risulta ridondante e confusa (il "seed" appare sia nel nome che nell'origine). Suggerito: "il codice di catena iniziale (alla radice dell'albero) è ricavato dal seed" oppure "il codice di catena radice è generato dal seed".
Il
...
figlia non può essere utilizzata per trovare la chiave genitore.
// [REVISIONE]: "La chiave figlia non può anche essere utilizzata" è un calco dall'inglese "The child key also cannot be used". In italiano la forma naturale è "La chiave figlia non può nemmeno essere utilizzata" oppure "Nemmeno la chiave figlia può essere utilizzata". La costruzione "non ... anche" è un anglicismo da evitare.
La chiave figlia non può anche
...
// [REVISIONE]: errore di accordo grammaticale — "lungo" è maschile ma il soggetto è "la chiave estesa" (femminile). Correggere in "è anche molto più lunga di altre stringhe codificate in base58check che abbiamo visto in precedenza."
...
// [REVISIONE]: "dispositivi di firma cold-storage o hardware" è una costruzione ambigua e poco naturale — il "cold-storage" non è un tipo di dispositivo di firma ma una modalità di conservazione. L'originale dice "cold-storage or hardware signing devices". Suggerito: "dispositivi di firma hardware o cold-storage" oppure "dispositivi hardware per la firma, usati in modalità cold storage". Inoltre "wallet di carta" (paper wallet) è un termine tecnico accettato ma andrebbe verificata la coerenza dell'uso nel resto del libro.
...
// [REVISIONE — DA RIVEDERE]: calco dall'inglese "widely used" — "diffusamente utilizzato" suona innaturale in italiano. Suggerito: "ampiamente diffuso", "molto usato" o "il diffuso BTCPay Server open source".
