...
BIP39 è stato proposto dall'azienda dietro il dispositivo di firma hardware Trezor e
...
funzione di key-stretching è rendere difficile costruire una tabella di ricerca
...
codice a 128 bit (64 bit), è plausibile che sarà in grado di attaccare
a forza bruta i restanti 64 bit. Se vede metà di un codice a 256 bit (128
bit), non è plausibile che possa attaccare a forza bruta l'altra metà. Non
...
possibilità pratica di attaccare a forza bruta o indovinare accidentalmente uno che
è in uso.
...
Nemmeno la chiave figlia può essere utilizzata per trovare eventuali fratelli. Se hai l'n-esimo figlio, non puoi trovare
...
è anche molto più lunga di altre stringhe codificate in base58check che abbiamo
visto in precedenza.
...
come il diffuso BTCPay Server open source.
