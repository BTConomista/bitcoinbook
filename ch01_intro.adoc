...
// [REVISIONE]: "compensazione delle operazioni" rende "clearing" — verificare uniformità con altri capitoli (in <<confirmations>> "clearing" è lasciato in inglese e in ch02 reso come "conferma"); valutare scelta unica per il termine.
...
// [REVISIONE]: "Raggiunto il blocco numero 1.411.200 circa, il cui raggiungimento è previsto per il 2035 circa" presenta una ridondanza ("raggiunto/raggiungimento") e doppio "circa"; riformulare es. "Intorno al blocco 1.411.200, atteso per il 2035, sarà stato emesso...". Inoltre "intrinsecamente deflazionistica" aggiunge una sfumatura assente nell'originale ("over the long term, the Bitcoin currency is deflationary") — verificare.
...
// [REVISIONE]: alterazione di significato — "global decentralized consensus on the valid blockchain" indica il consenso su QUALE blockchain è valida (la scelta tra catene candidate), non sulla "validità della blockchain". Suggerito: "sulla blockchain valida".
...
// [REVISIONE]: nel paragrafo seguente l'originale dice "As a developer, I see Bitcoin..." senza nominare l'autore: il libro ha due autori (Antonopoulos e Harding) e specificare "Andreas" è un'interpretazione del traduttore. Valutare se mantenere la prima persona ("Come sviluppatore, vedo...") o uniformarsi alla resa adottata altrove quando l'autore parla in prima persona (es. "I (Andreas)" più sotto in ch01).
...
// [REVISIONE]: errore tecnico — l'originale dice "currency issuance or settlement and validation of transactions": "settlement" è il regolamento/compensazione delle transazioni, non la "regolamentazione" (= regulation, attività normativa). Sostituire "regolamentazione" con un termine come "regolamento", "compensazione" o "settlement", coerente con il resto del libro.
...
Satoshi Nakamoto.], scritto sotto lo pseudonimo di Satoshi Nakamoto (vedi appendice A <<satoshi_whitepaper>>). Nakamoto ha unito diverse invenzioni precedenti, come le firme digitali e Hashcash, per creare un sistema di moneta elettronica completamente decentralizzato che non dipende da un'autorità centrale per l'emissione di valuta, la regolamentazione o la validazione delle transazioni. Un'innovazione chiave è stata l'uso di un sistema di calcolo distribuito (chiamato algoritmo di "proof-of-work") per condurre una sorta di lotteria globale ogni 10 minuti, in media, permettendo alla rete decentralizzata di raggiungere un _consenso_ sullo stato delle transazioni. Questo risolve elegantemente il problema della doppia spesa (double-spend), in cui un'unità di valuta può essere spesa due volte. In precedenza, il problema della doppia spesa era una debolezza delle valute digitali e veniva affrontato facendo passare tutte le transazioni attraverso un centro di smistamento centrale.

// [REVISIONE]: errore tecnico — l'originale dice "based on a reference implementation": il riferimento è all'implementazione di riferimento (il codice), non al whitepaper. È lo stesso errore concettuale già segnalato in <<choosing_wallet>> per "basata sul documento originale di Satoshi Nakamoto", ma questa è una seconda occorrenza separata. Sostituire "sul documento di Nakamoto" con "sull'implementazione di riferimento" o equivalente.
...
// [REVISIONE]: errore tecnico — l'originale dice "derived from the original implementation written by Satoshi Nakamoto" (il codice/implementazione, non il whitepaper). La traduzione "basata sul documento originale" confonde l'implementazione di riferimento con il whitepaper. Sostituire "documento" con "implementazione" o "codice originale".
...
- Client leggero:
// [REVISIONE]: omessa una parte tecnica — l'originale ("for receiving and sending Bitcoin transaction information") indica che il client SPV non solo riceve, ma anche invia (broadcasta) informazioni di transazione tramite il full node; la traduzione attuale ("per ottenere le informazioni") riporta solo la ricezione.
// [REVISIONE]: errore di nomenclatura — l'acronimo SPV sta per "Simplified Payment Verification" (originale: "simplified-payment-verification"), non "Simple Payment Verification". Correggere "Simple" in "Simplified".
...
// [REVISIONE]: aggiunta non presente nell'originale — l'inglese cita solo "third-party API client". L'inciso "(o "wallet ospitato")" introduce un sinonimo non equivalente: un wallet custodial/ospitato non coincide necessariamente con un client API di terze parti. Valutare se rimuovere o riformulare.
...
[TIP]
====
// [REVISIONE]: aggiunta "(ad esempio, per controllare che forniscano tutti la stessa informazione)" non presente nell'originale; inoltre "dell'onestà dei peer" attenua "the integrity of its peers" (integrità in senso tecnico), valutare resa più aderente.
...
// [REVISIONE]: aggiunta editoriale non presente nell'originale ("See <<recovery_code_sample>> for examples.") — l'inciso "(tabella 1.1)" e l'aggiunta di "(1.1)" alla `<caption>` introducono una numerazione manuale che la toolchain non genera automaticamente da `<<...>>`; stesso pattern dei "Figura 1/Figura 2" segnalati più sotto. Verificare se rimuovere "(tabella 1.1)" e "(1.1)" dalla didascalia per coerenza.
...
// [REVISIONE]: amplificazione editoriale del [TIP] originale — il blocco è stato esteso oltre il testo inglese, in particolare con l'inciso "Inoltre, tutte le transazioni sulla blockchain Bitcoin sono pubbliche. Se un indirizzo viene collegato alla tua identità, chiunque può potenzialmente tracciare tutti i pagamenti inviati e ricevuti da quell'indirizzo." Questa generalizzazione va oltre l'originale che dice solo "If you post your address publicly, everyone will be able to see how much bitcoin other people sent to that address." Valutare se ridurre l'amplificazione e attenersi al testo originale.
[TIP]
====
...
// [REVISIONE]: aggiunta editoriale "Figura 1" non presente nell'originale ("see <<wallet_receive>>"); stesso pattern alla riga "(si veda la <<wallet-send>> Figura 2)". I numeri di figura sono fragili rispetto a riordinamenti del libro — verificare se mantenere solo <<wallet_receive>> e <<wallet-send>>.
==== Ricevere Bitcoin

Alice preme il pulsante _Ricevi_, che fa visualizzare un codice QR (si veda la <<wallet_receive>>, Figura 1).
...
// [REVISIONE]: errore di traduzione del titolo — l'originale è "Getting Your First Bitcoin" (acquisire/ottenere), non "Detenere" (=holding); l'errore si propaga in ch02 nel rimando a questa sezione. Suggerito: "Ottenere i tuoi primi bitcoin".
==== Detenere i tuoi primi bitcoin
// [REVISIONE]: omissione — l'elenco originale dei pagamenti reversibili ("credit card, debit card, PayPal, and bank transfer") è stato compresso a "carte di credito, PayPal, ecc.": ripristinare le quattro voci (carte di credito, carte di debito, PayPal e bonifici bancari).
// [REVISIONE]: omessa la frase finale del paragrafo originale: "As a new user, this means you cannot buy bitcoin instantly with a credit card. With a bit of patience and creative thinking, however, you won't need to." — reintegrarla a chiusura del paragrafo prima dell'elenco dei metodi di acquisto.
...
// [REVISIONE]: markup AsciiDoc rotto — i quattro elementi seguenti iniziano con `*` senza spazio (es. `*Da un amico` invece di `* Da un amico`); di conseguenza non vengono renderizzati come bullet di un elenco puntato, ma come blocchi `bold` non chiusi. Inserire uno spazio dopo l'asterisco di apertura per ognuno dei quattro elementi.
...
// [REVISIONE]: due imprecisioni tecniche/terminologiche nel paragrafo seguente. (1) "floating exchange rate" — termine economico tecnico in italics nell'originale — è reso con "tasso di cambio che varia continuamente"; sarebbe più preciso "tasso di cambio fluttuante". (2) "the price tends to fluctuate minutely several times per second" → "il prezzo può cambiare rapidamente, anche più volte al secondo": "minutely" qui significa "in misura molto piccola/leggera", non "rapidamente". La traduzione perde la sfumatura che le oscillazioni sono frequenti ma minime.
...
.Conferme
****
// [REVISIONE]: aggiunta editoriale "(capitolo 12)" non presente nell'originale; il numero di capitolo è fragile rispetto a riordinamenti — verificare se mantenere solo `<<mining>>`.
