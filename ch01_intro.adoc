...
Se l’applicazione ti chiede il codice di recupero in qualsiasi altro momento, consulta un esperto per assicurarti di non essere vittima di un((("wallets", "recovery codes", startref="wallet-recovery")))((("recovery codes", startref="recovery-code"))) attacco di phishing.
====
...
[TIP]
====
Esistono diversi formati di indirizzi Bitcoin e ordini di pagamento (invoice)((("invoices"))). Gli indirizzi e gli ordini di pagamento possono essere condivisi con altri utenti Bitcoin, che potranno usarli per inviarti bitcoin direttamente nel tuo wallet.
...
Alice ((("bitcoins", "spending", id="bitcoin-send")))((("spending bitcoins", id="send-bitcoin")))((("bitcoins", "receiving", id="bitcoin-receive")))((("receiving bitcoins", id="receive-bitcoin")))ha deciso di acquistare 0,001 bitcoin. Dopo aver controllato il tasso di cambio con Joe, gli consegna la somma corrispondente in contanti, apre l'applicazione del suo wallet mobile e seleziona l'opzione Ricevi.
